...
class DetailPage extends React.Component {
  ...
  cashValid = (cash) => {
    const [integer, decimal] = parseFloat(cash).toString().split('.');
  ...
  checkSubmitValidate = () => {
    ...
    const { is_description, description } = detailInfo;
    if (is_description === 1) {
      if (!description || !description.trim()) {
        message.error('請填寫說明文字!');
        return false;
      }
      // const count = this.calcCharCount(description);
      // if (description.length > 24) {
      //   message.error('說明文字長度超長');
      //   return;
      // }
    }
  ...
  handlePublish = (type) => {
    ...
    const self = this;
    const content = '提交的內容將立刻發布並展示在獎賞列表\n你還要繼續嗎？';
    confirm({
      ...
      <div className="p-detail-wrap">
        ...
        >
          <Form>
            ...
            <FormItem {...formItemLayout} label="說明文字">
              <div>
                <RadioGroup
                  onChange={this.handleRadioChange.bind(this, 'is_description')}
                  disabled={isDisabledEdit}
                  value={detailInfo.is_description}
                >
                  <Radio value={1}>需要</Radio>
                  <Radio value={0}>不需要</Radio>
                </RadioGroup>
              </div>
              {detailInfo.is_description === 1 ? (
                <Input.TextArea
                  disabled={isDisabledEdit}
                  placeholder="請簡單填寫說明文字(24個字符以內)"
                  maxLength={24}
            ...
            <FormItem {...formItemLayout} label="上傳圖片">
              <UploadByDragger
                ...
                limitWidthHeight={{ width: 520, height: 390 }}
                showUploadList={{
