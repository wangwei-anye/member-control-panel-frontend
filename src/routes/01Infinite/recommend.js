...
class RecommedListPage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      customOrder: '',
  ...
  handleToDetail = (record, type = 'look') => {
    ...
    this.props.history.push(
      `/01-infinite/recommend/detail?id=${record.id}&type=${type}`
    );
  };
      ...
      onOk() {
        return new Promise((resolve) => {
          ...
          self.props.history.push(
            `/01-infinite/recommend/detail?id=${record.id}&type=edit`
          );
        });
      },
    });
  ...
  handleAddShopClick = () => {
    this.props.history.push('/01-infinite/recommend/detail');
  };
  ...
  handleSearch = () => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (values.off_at && values.off_at.length) {
        query.off_shelf_start_time = moment(values.off_at[0])
          .set({ second: 0 })
          .format('YYYY-MM-DD HH:mm:ss');
        query.off_shelf_end_time = moment(values.off_at[1])
          .set({ second: 59 })
          .format('YYYY-MM-DD HH:mm:ss');
      }
      query.page = 1;
  ...
  stopGrant = async (type) => {
    const { selectedRowKeys } = this.state;
    const { recommedInfo } = this.props.infinite;
    const { list } = recommedInfo;
    if (selectedRowKeys.length === 0) {
      message.success('請至少選擇一行進行操作');
      return;
    }
    confirm({
      icon: 'info-circle',
      title: type === 'publish' ? '正在操作批量上架' : '正在操作批量下架',
      content: '操作不可取消，請確認',
      onOk: async () => {
        try {
          const { data } = await this.props.dispatch({
            type: 'infinite/bitchPublishOrCancelRequest',
            payload: {
  ...
  render() {
    const { recommedInfo } = this.props.infinite;
    const { list, total, loading } = recommedInfo;
    ...
    return (
      <div className="p-recommendlist-wrap">
        <FoldableCard title={<span>搜索條件</span>}>
          <Form>
            <Row className="form-control-row" gutter={12}>
              <Col span={8}>
