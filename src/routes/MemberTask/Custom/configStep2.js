...
class ConfigStep2Page extends React.Component {
  ...
  async componentDidMount() {
    ...
    let fileName = '';
    if (data.status) {
      const detailInfo = data.data.offer_task_config;
      ...
      const entryInfo = Object.assign({}, this.state.entryInfo, detailInfo, {
        start_time: detailInfo.start_time
          ? moment(detailInfo.start_time).format('YYYY-MM-DD HH:mm')
          : moment().add(1, 'day').format('YYYY-MM-DD HH:mm'),
        end_time: detailInfo.end_time
          ? moment(detailInfo.end_time).format('YYYY-MM-DD HH:mm')
          : moment().add(8, 'day').format('YYYY-MM-DD HH:mm'),
      });
  ...
  async fetchAccountListByPartment(pid) {
    ...
    if (res.data.status && res.data.data) {
      ...
      const { offer_account_status, offer_account, offer_account_name } =
  ...
  async nextAction(type) {
    if (type === 'look') {
      ...
      this.props.history.push(
        '/member-task/config/rule?id=' + id + '&type=look'
      );
    } else {
      const formData = this.formatData();
      if (formData) {
        const { id } = this.state;
        const { offer_account_status, offer_account } = formData;
        if (
  ...
  gotoRule = (returnId) => {
    if (this.state.editType === 'edit') {
      this.props.history.push(
        '/member-task/config/rule?id=' + returnId + '&type=edit'
      );
    } else {
      this.props.history.push('/member-task/config/rule?id=' + returnId);
    }
  };
