...
class CreateEventCom extends React.Component {
  ...
  async fetchEventDescByReportChange(id) {
    ...
    this.setState({
      reportChannelDescList: [],
      getEventDescFlag: true,
    });
    // NOTE: 这里根据后端的要求 trigger_type 参数写死为 0
    const {
      data: { data, status },
    } = await fetchEventListRequest({
    ...
    if (status) {
      await this.setState({
        reportChannelDescList: data.list || [],
        getEventDescFlag: false,
      });
    } else {
  ...
  modalOkAction = () => {
    const { event_desc, reportChannelDescList, event_params } = this.state;
    const { eventInfo } = this.props;
              ...
              <Col span={16}>
                {item.type === 'abs' ? (
                  <Input
                    ...
                    value={item.value[0]}
                    onChange={(value) =>
                      this.rangeChangeAction(value.target.value, 'range1', idx)
                    }
                  />
                ) : (
                  ...
                  <div>
                    <InputNumber
                      ...
                      style={{ width: '130px' }}
                      onChange={(value) =>
                        this.rangeChangeAction(value, 'range1', idx)
                      }
                      value={item.value[0]}
                      disabled={this.props.isLook}
                    />
                    ...
                    <InputNumber
                      ...
                      disabled={this.props.isLook}
                      onChange={(value) =>
                        this.rangeChangeAction(value, 'range2', idx)
                      }
                      style={{ width: '129px' }}
                      value={item.value[1]}
                    />
  ...
  render() {
    const { reportChannelDescList, event_desc, event_params } = this.state;
    const { eventInfo } = this.props;
    const { reportChannelList } = this.props.system;
        ...
        footer={
          this.props.isLook
            ? [
              <Button
                key="back"
                onClick={this.modalCancelAction}
            ...
            : [
                ...
                <Button
                  ...
                  disabled={this.props.isLoading}
                  type="primary"
                  loading={this.props.isLoading || this.state.getEventDescFlag}
                  onClick={this.modalOkAction}
                >
                  确定
                </Button>,
              ]
