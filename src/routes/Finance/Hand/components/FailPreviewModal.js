...
const Cell = props => {
  ...
  if (props.cell.width) {
    style.backgroundCwidtholor = props.cell.width;
  }
  return (
...
export default class FailPreviewModal extends React.Component {
  constructor(props) {
    super(props);
  ...
  buildTitle = temp => {
    ...
    title.push(
      ...
      { value: '發放狀態', readOnly: true },
      { value: '失敗原因', readOnly: true, width: 350 }
    );
    return title;
  };
  buildItem = (element, index) => {
    ...
    result.push(
      ...
      { value: '發放失敗', readOnly: true },
      { value: element.reason, readOnly: true, width: 350 }
    );
    return result;
  };
  buildList = () => {
